...
namespace RealTimePointsProcessFunctionApp.Implementation
{
    public class SfmcRepo : ISfmcRepo
    {
        ...
        ILogger<SfmcRepo> _log;
        public SfmcRepo(SalesForceAPIMService salesForceAPIMService, IConfiguration configuration, ILogger<SfmcRepo> logger)
        {
            salesForceService    = salesForceAPIMService ?? throw new ArgumentNullException(nameof(salesForceAPIMService));
            SfmcDataExtensionKey = "SEG_All_Cust_Data_QA";//Environment.GetEnvironmentVariable("SEG_Key") ?? throw new ArgumentNullException(nameof(SfmcDataExtensionKey));
