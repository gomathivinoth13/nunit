...
namespace RealTimePointsProcessFunctionApp.Functions
{
    ...
    public class RealTimePoint
    {
        ...
        public RealTimePoint(ISfmcRepo sfmcRepo, ILogger<RealTimePoint> logger, EagleEyeProcess eagleEyeProcess, EagleEyeService eagleEyeService)
        {
         ...
         ILogger<RealTimePoint> logger)
        {
            try
            {
                ...
                foreach (var data in dataArray)
                {
                    if (!string.IsNullOrEmpty(data.WalletID))
                    {
                        ...
                        if (pointsResult != null && pointsResult.IsSuccessful && pointsResult.Result != null && pointsResult.Result.Results != null && pointsResult.Result.Results.Count > 0)
                        {
                            ...
                            if (memberId != null)
                            {
                                ...
                                if (point != null)
                                {
                                    var dataExtentionsResponse = await _sfmcRepo.RealTimeDataProcess(point);
                    ...
                    {
                        _log.LogWarning(ResponseMessage.NullErrorMessage);
                        _httpResponseData = SetErrorResponse(HttpStatusCode.BadRequest, ResponseMessage.InvalidInput, req);
                    }
                ...
                {
                    return SetErrorResponse(HttpStatusCode.InternalServerError, ResponseMessage.InsertionNotSuucessful, req);
                }
            }
            catch (Exception ex)
            {
