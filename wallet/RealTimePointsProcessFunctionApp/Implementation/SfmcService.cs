...
namespace RealTimePointsProcessFunctionApp.Implementation
{
    public class SfmcService : ISfmcService
    {
        ...
        private readonly string SEG_Key;
        ILogger<SfmcService> _log;
        public SfmcService(SalesForceAPIMService salesForceAPIMService, IConfiguration configuration,ILogger<SfmcService> log)
        {
            salesForceService = salesForceAPIMService;
            SEG_Key = "SEG_All_Cust_Data_QA"; //Environment.GetEnvironmentVariable("SEG_Key");
            _log = log;
        }

        public async Task<DataExtentionsResponse> CreateInputItem(int Current_point_balance, string member_id, int Expiring_points, DateTime Next_exp_date)
        {
        ...
        public async Task<DataExtentionsResponse> InsertToSfmc(DataExtentionsRequest dataExtentionRequest)
        {
            try
            {
                ...
                if (dataExtentionRequest.items.Count > 0)
                {
                    if (SEG_Key != null)
                    {
                        dataExtentionsResponse = await salesForceService.UpsertAsync(dataExtentionRequest, SEG_Key);
                        
                    }
                    else
                    {
                        _log.LogWarning(ResponseMessage.ConfigValueNullErrorMessage);
                    }
                }
                else
                {
        ...
        public  Task<DataExtentionsResponse> RealTimeDataProcess(Account account)
        {
            ...
            if (account.PointsInfo != null)
            {
                ...
                var dataExtentionsResponse = CreateInputItem(item.Expiring_Points, item.MEMBER_ID, item.Current_Points_Balance, item.Next_Exprn_Dt);
                return dataExtentionsResponse;
            }
            else
            {
                var dataExtentionsResponse = CreateInputItem(0, item.MEMBER_ID, 0, DateTime.Parse("01/01/2000 10:00:00 AM"));
